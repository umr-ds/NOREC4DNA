...
import pytest
from norec4dna import LTDecoder, OnlineDecoder, RU10Decoder
...
@pytest.mark.parametrize(
    "args", [("test_files/RU10_logo.jpg", False, 720), ("test_files/RU10_logoH.jpg", True, 721)]
)
def test_ru10(args):
    x = RU10Decoder(
        args[0], use_headerchunk=args[1], error_correction=nocode, static_number_of_chunks=args[2]
    )
...
@pytest.mark.parametrize(
    "args", [("test_files/ONLINE_logo.jpg", False, 231), ("test_files/ONLINE_logoH.jpg", True, 231)]
)
def test_online(args):
    decoder = OnlineDecoder(
        args[0], error_correction=nocode, use_headerchunk=args[1], static_number_of_chunks=args[2]
    )
    decoder.decode(
...
@pytest.mark.parametrize(
    "args", [("test_files/LT_logo.jpg", False, 720), ("test_files/LT_logoH.jpg", True, 721)]
)
def test_lt(args):
