...
import pandas as pd
from norec4dna import OnlineDecoder, RU10Decoder, RU10Encoder
...
if __name__ == "__main__":
    ...
    encoder = RU10Encoder(
        file,
        number_of_chunks,
        RaptorDistribution(number_of_chunks=number_of_chunks),
    ...
    save_prefix = (
        ...
        + "_"
        + ("H" if insert_header else "")
        + "_"
        + ("R" if rules is not None else "")
        + "_"
        + error_correction_str
        + "_"
        + str(norepair_symbols)
        + "_"
        + PACKET_LEN_FORMAT
        + "_"
        + CRC_LEN_FORMAT
        ...
        + ID_LEN_FORMAT
        + "_"
        + ("NUMC" if save_number_of_chunks_in_packet else "")
        + "_"
        + str(upper_bound)
    )
