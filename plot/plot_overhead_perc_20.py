...
df.sort_values(by=["codecName", "numberOfEncodedPackets", "number_of_chunks"], inplace=True)
...
for name, group in df.groupby(["codecName"]):
    df2 = group.groupby(["OverheadPercent"]).mean()
...
plt.tight_layout()
plt.show(block=False)
plt.savefig("../pdfs/overhead_percent_vs_res_20" + ".pdf", bbox_inches="tight")
plt.savefig("../pdfs/overhead_percent_vs_res_20" + ".svg")
