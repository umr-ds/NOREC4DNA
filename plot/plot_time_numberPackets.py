...
plt.rc("axes", prop_cycle=(
        cycler("color", ["m", "m", "r", "r", "r", "r", "g", "g", "g", "g", "b", "b", "b", "b", "y", "y", "y", "y",
...
axs.set_ylabel("Duration (seconds)")
for name, group in df.groupby(["codecName"]):
    df2 = group.groupby(["numberOfEncodedPackets"]).mean()
    tmp = df.loc[df["codecName"] == name]
    m = tmp["number_of_chunks"].mean()
    tmp1 = df2.reset_index()
    tmp1.plot(x="numberOfEncodedPackets", y="timeNeeded", label=name.replace("_", " "), ax=axs, legend=True, )
...
plt.tight_layout()
plt.show(block=False)
plt.savefig("../pdfs/time_vs_numberOfEncoded_" + ".pdf", bbox_inches="tight")
plt.savefig("../pdfs/time_vs_numberOfEncoded_" + ".svg")
