...
class OnlineEncoder(Encoder):
    ...
    def generate_new_checkblock_id(self) -> int:
        ...
        max_num = int(
            Encoder.calc_max_size(struct.calcsize("<" + self.check_block_number_len_format))
        )
    ...
    def choose_packet_numbers(self, degree: int, seed: int) -> typing.Set[int]:
        ...
        for _ in range(0, degree):
            tmp: int = int(self.rng.choice(range(0, len(self.chunks) + len(self.auxBlocks))))
            while tmp in res:
                tmp = int(self.rng.choice(range(0, len(self.chunks) + len(self.auxBlocks))))
            res.add(tmp)
        return res
