...
def main(
    ...
    packets_to_create=None,
    save_as_zip=DEFAULT_SAVE_AS_ZIP,
    xor_by_seed=False,
    id_spacing=0,
    custom_dist=None,
):
    ...
    a = p.map(
        partial(
            ...
            append=append,
            insert_header=insert_header,
            seed_len_format=_seed_size_str,
            method=method,
            mode1bmp=mode1bmp,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--store_as_zip",
        required=False,
        default=DEFAULT_SAVE_AS_ZIP,
        action="store_true",
        help="if set, store result as zip file",
    )
    ...
    if _chunk_size != 0:
        _number_of_hunks = Encoder.get_number_of_chunks_for_file_with_chunk_size(
            _input_file, _chunk_size
        )
    if _list_size < _out_size:
        print("Warning: A list size smaller than the out_size may result in non-optimal results.")
    if _optimization:
        print(
            "Warning: Please make sure to generate enough packets for the automated optimization."
        )
    ...
    if not _optimization:
        if _number_of_splits > 1:
            ...
            prepend_matching = {
                input_files[i]: number_to_base_str(i, power_of_four)
        ...
        for _input_file in input_files:
            try:
                ...
                main(
                    ...
                    _spare1core,
                    method=_method,
                    append=prepend_matching[_input_file],
                    insert_header=_insert_header,
                    seed_len_format=_seed_size_str,
                    drop_above=_drop_above,
                    save_as_fasta=_store_as_fasta,
                    save_as_zip=_store_as_zip,
        ...
        par_packets, _out_file, _file_ending = main(
            _input_file,
            _repair_symbols,
            _list_size,
            _out_size,
        ...
        optimized_packets = optimizer.automated_optimization(
            _overhead, _overhead_factor, _error_prob_factor, plot=_plot
        )
